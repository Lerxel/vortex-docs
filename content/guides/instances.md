...
`Instances` are the basic building blocks for objects in a game. Each concrete class has its own [`Properties`](./properties.md) and methods.

You can create a new `Instance` using the [`Instance.new`](../reference/globals/instance-new.md) method in a script, by passing in the `ClassName` of the `Instance` as the first argument.
